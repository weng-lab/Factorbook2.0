...
export default function MotifTile({ motif, species, selectedExperimentID, selectedPeakID }: { motif: any, species: string, selectedExperimentID: string, selectedPeakID: string }) {
  ...
  const isGreyedOut = poorPeakCentrality(motif) || poorPeakEnrichment(motif);
    ...
    >
      ...
      <DialogActions>
        ...
        <Button
          ...
          onClick={() => {
            if (exportMotif) {
              handleDownload(motif.id, motifppm, svgRef.current);
            }
            if (exportPeakSites) {
            ...
            if (exportLogo && svgRef.current) {
              downloadSVGElementAsSVG({ current: svgRef.current }, `${motif.name}-logo.svg`);
            }
            setIsDialogOpen(false);
          }}
        >
