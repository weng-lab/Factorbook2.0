...
const MotifCell: React.FC<{ chromosome: string; start: number; end: number }> = ({ chromosome, start, end }) => {
  ...
  return (
      <div>
          {loading && <img style={{ width: '16px', height: '16px' }} src={"/loader.svg"} alt={'Loading'} />}
          ...
          <StyledSearchBox>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const start = +value.split(":")[1].split("-")[0].replaceAll(",","")
                const end = +value.split(":")[1].split("-")[1].replaceAll(",","")
                setRegions([{ chromosome: chromosome, start: start!!, end: end!! }])
