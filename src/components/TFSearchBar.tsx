    ...
    if (tfSuggestion && tfSuggestion.length > 0) {
      ...
      const snp = tfSuggestion.map(
        (g: {
          ...
          label: (!tfAassignment || !tfAassignment.get(g.name!)) ? ""           
          : (tfAassignment.get(g.name!)["TF assessment"] as string).includes("Likely")
          ? "Likely sequence-specific TF - "
          : SEQUENCE_SPECIFIC.has(tfAassignment.get(g.name!)["TF assessment"])
    ...
    <Box>
                <Stack direction="row" spacing={2}>
                    ...
                    <StyledAutocomplete
                      ...
                      onKeyDown={(event: any) => {
                        if (event.key === "Enter" && snpValue) {
                          ...
                          window.open(snpValue ? `/TranscriptionFactor/${ assembly==="GRCh38" ? "human" : "mouse" }/${snpValue}/Function`: "", "_self")
                        }
                      }}
                        ...
                        return (
                          <li {...props} key={option}>
                            <Grid2 container alignItems="center">
                              ...
                              >
                                <Box component="span">{formatFactorName(option, assembly)}</Box>
                                {selectedSnp && (
                                  <Typography
                                    variant="body2"
                  ...
                  //  onClick={handleSubmit}
                   ...
                   href={snpValue ? `/TranscriptionFactor/${ assembly==="GRCh38" ? "human" : "mouse" }/${snpValue}/Function`: ""}
                  >
                    Search
                  </Button>
                </Stack>
