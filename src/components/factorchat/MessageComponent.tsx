...
import { BackendMessage, FactorChatFigure, Message, Role } from "./types"
...
export const MessageComponent = <T extends Role>(message: Message<T>) => {
  const isUser = message.role === "user"
  const textToDisplay = isUser ? message.contents as string : (message.contents as BackendMessage).text
  const error = isUser ? false : (message.contents as BackendMessage).error
  const figures = isUser ? [] : (message.contents as BackendMessage).figures
  const files = useMemo(() => isUser ? [] : (message.contents as BackendMessage).files, [])
  ...
  return (
    <MessageBubble isUser={isUser}>
      ...
      {/* Disabling "Thoughts" Accordion for now */}
      {/* {!isUser && !error &&
        <div>
