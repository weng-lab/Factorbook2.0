...
import { Button, CircularProgress, Typography, Box, Grid, Alert } from "@mui/material";
...
const MotifIconsRow: React.FC<MMotif> = (x) => {
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={2}>
        <FileDownloadOutlinedIcon onClick={() => downloadSVG(r, "logo.svg")}>Download</FileDownloadOutlinedIcon>
        <SwapHorizIcon onClick={() => setRC(!rrc)}>Reverse Complement</SwapHorizIcon>
      </Grid>
    </Grid>
  );
};
...
const COLUMNS = (title: string) => { 
  ...
  {
    header: "",
    value: (x: MMotif) => (isPWMObjectArray(x.pwm) ? x.pwm[0].A : x.pwm[0][0]),
  ...
  },
  {
    ...
    render: (x: MMotif) => 
     <Link style={{ color: "#8169BF" }} href={`/TranscriptionFactor/Human/${x.factor.split("phospho")[0]}/Function`}  rel="noopener noreferrer" target="_blank">
            {x.factor}
          </Link>,
  },
  {
...
const MotifUMAP: React.FC<{ url: string; title: string }> = (props) => {
  ...
  return data.length === 0 ? (
    ...
    <Box style={{padding: "30px"}}>
      <br/>
      <Alert  icon={<InfoOutlinedIcon fontSize="inherit" style={{color:"#5056A9"}} />} severity="info" 
    sx={{
    borderRadius: '24px', border: '1px solid rgba(79, 55, 138, 0.16)', padding: '8px', background: '#F2F0FF'    
    }}
  >
    
<Typography variant="h6" gutterBottom style={{ fontFamily: 'Helvetica Neue', color: '#5056A9',
fontSize: '14px'}}>
        <b>This view displays a UMAP projection of {points.length.toLocaleString()}{" "}
        motifs discovered by{" "}
        {props.title === "meme"
          ? "MEME on ChIP-seq datasets"
          : "the ZMotif neural network on HT-SELEX datasets"}
        . </b> <>Hold shift, click, and drag to view information about motif clusters
      ...
      <Grid container spacing={2}>
        ...
        <Chart
          marginFraction={0.12}
          innerSize={{ width: 1100, height: 900 }}
                        domain={domain}
